...
if [ "$CMD" = "start" ]; then
  CURRENT_PID=`ps axf | grep $FLEX_HOME/bin/portal32.jar | grep -v grep | awk '{ printf $1 }'`
  echo $CURRENT_PID
  ...
  nohup java  $JVM_MEM_OPTS -jar "$FLEX_HOME"/bin/portal32.jar $JAVA_OPTS $SPRING_OPTS > ../logs/catalina.out &

  
  tail -f ../logs/catalina.out
elif [ "$CMD" = "stop" ]; then
  export FLEX_HOME="$FLEX_HOME"
  #kill -term `cat "$FLEX_HOME"/portal32.pid`
  ps axf | grep $FLEX_HOME/bin/portal32.jar | grep -v grep | awk '{print "kill -term " $1}' | sh
  exit 0
  ...
  echo "Using Spring process options: $SPRING_OPTS"
  #nohup java  $JVM_MEM_OPTS -jar "$FLEX_HOME"/bin/portal32.jar $JAVA_OPTS $SPRING_OPTS > ../logs/catalina.out &
  java -cp "$FLEX_HOME/bin/portal32.jar:$CLASSPATH" $JVM_MEM_OPTS $JAVA_OPTS $SPRING_OPTS > ../logs/catalina.out &
  tail -f ../logs/catalina.out
  exit 0
